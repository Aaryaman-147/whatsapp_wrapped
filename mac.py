...
Usage:
    ...
    python3 whatsapp_wrapped.py --db /path/to/ChatStorage.sqlite
"""
...
# WhatsApp database locations (macOS - try in order)
WHATSAPP_PATHS = [
...
def get_candidate_paths():
    """Return platform-specific candidate DB paths."""
    system = platform.system()
    paths = []

    if system == "Darwin":
        # Use the existing macOS paths
        return WHATSAPP_PATHS

    if system == "Windows":
        # Very best-effort guesses – WhatsApp Desktop paths change over time
        appdata = os.getenv("APPDATA")        # Roaming
        localappdata = os.getenv("LOCALAPPDATA")

        if appdata:
            # Older/standalone WhatsApp Desktop
            paths.append(os.path.join(appdata, "WhatsApp", "ChatStorage.sqlite"))

        if localappdata:
            # Microsoft Store version – look under Packages
    ...
    # Other OS: no default paths
    return []

def find_database():
    """Find the WhatsApp database path from known locations."""
...
def get_name(jid, contacts):
    ...
    if '@' in jid:
        phone = jid.split('@')[0]
        # Format as phone number (US-ish formatting, just cosmetic)
        if len(phone) == 10:
...
def check_access(db_override=None):
    """Resolve and validate the database path."""
    global WHATSAPP_DB

    if db_override:
        if not os.path.exists(db_override):
            print(f"\n[FATAL] Database file not found at:\n   {db_override}")
            sys.exit(1)
        WHATSAPP_DB = db_override
    else:
        WHATSAPP_DB = find_database()

    if not WHATSAPP_DB:
        print("\n[FATAL] WhatsApp database not found automatically.")
        print("   You can:")
        print("   1) On macOS: let the script use the built-in detection paths.")
        print("   2) On Windows: locate ChatStorage.sqlite yourself and pass it with --db.")
        print("\n   Searched locations:")
        for path in get_candidate_paths():
            print(f"   - {path}")
        sys.exit(1)
    ...
    try:
        ...
        print(f"   Path: {WHATSAPP_DB}")
        print(f"   Error: {e}")

        if platform.system() == "Darwin":
            print("\n   On macOS, you likely need Full Disk Access:")
            ...
            print("   - WhatsApp is closed, so the DB isn't locked.")
            print("   - You have read access to the file.")
            print("   - The file really is an iOS-style ChatStorage.sqlite with ZWAMESSAGE table.")
        sys.exit(1)
...
def analyze(ts_start, ts_jun):
    ...
    # Peak hour
    r = q(
        f"SELECT CAST(strftime('%H',datetime(ZMESSAGEDATE+{COCOA_OFFSET},'unixepoch','localtime')) AS INT) h, COUNT(*) c "
        f"FROM ZWAMESSAGE WHERE ZMESSAGEDATE>{ts_start} GROUP BY h ORDER BY c DESC LIMIT 1"
    )
    ...
    r = q(
        f"SELECT CAST(strftime('%w',datetime(ZMESSAGEDATE+{COCOA_OFFSET},'unixepoch','localtime')) AS INT) d, COUNT(*) "
        f"FROM ZWAMESSAGE WHERE ZMESSAGEDATE>{ts_start} GROUP BY d ORDER BY 2 DESC LIMIT 1"
    )
    ...
    for e in emojis:
        r = q(
            f"SELECT COUNT(*) FROM ZWAMESSAGE WHERE ZTEXT LIKE '%{e}%' "
            f"AND ZMESSAGEDATE>{ts_start} AND ZISFROMME=1"
        )
        counts[e] = r[0][0]
    d['emoji'] = sorted(counts.items(), key=lambda x:-x[1])[:5]

#new part
            # GIF & sticker usage (from ZWAMEDIAITEM)
    # Assumes ZWAMESSAGE.ZMEDIAITEM → ZWAMEDIAITEM.Z_PK
    r = q(f"""
        SELECT
            SUM(CASE WHEN m.ZISFROMME=1 AND (mi.ZVCARDSTRING LIKE '%gif%' OR mi.ZMEDIALOCALPATH LIKE '%.gif%') THEN 1 ELSE 0 END) AS gif_sent,
            SUM(CASE WHEN m.ZISFROMME=0 AND (mi.ZVCARDSTRING LIKE '%gif%' OR mi.ZMEDIALOCALPATH LIKE '%.gif%') THEN 1 ELSE 0 END) AS gif_recv,
            SUM(CASE WHEN m.ZISFROMME=1 AND (mi.ZVCARDSTRING LIKE '%webp%' OR mi.ZMEDIALOCALPATH LIKE '%Sticker%' OR mi.ZMEDIALOCALPATH LIKE '%.webp%') THEN 1 ELSE 0 END) AS sticker_sent,
            SUM(CASE WHEN m.ZISFROMME=0 AND (mi.ZVCARDSTRING LIKE '%webp%' OR mi.ZMEDIALOCALPATH LIKE '%Sticker%' OR mi.ZMEDIALOCALPATH LIKE '%.webp%') THEN 1 ELSE 0 END) AS sticker_recv
        FROM ZWAMESSAGE m
        JOIN ZWAMEDIAITEM mi ON mi.Z_PK = m.ZMEDIAITEM
        WHERE m.ZMESSAGEDATE>{ts_start}
    """)
    if r:
        gif_sent, gif_recv, sticker_sent, sticker_recv = [x or 0 for x in r[0]]
    else:
        gif_sent = gif_recv = sticker_sent = sticker_recv = 0

    d['media_usage'] = {
        'gif_sent': gif_sent,
        'gif_recv': gif_recv,
        'sticker_sent': sticker_sent,
        'sticker_recv': sticker_recv
    }
...
# new part
    ...
    # Busiest day
    r = q(
        f"SELECT DATE(datetime(ZMESSAGEDATE+{COCOA_OFFSET},'unixepoch','localtime')) d, COUNT(*) c "
        f"FROM ZWAMESSAGE WHERE ZMESSAGEDATE>{ts_start} GROUP BY d ORDER BY c DESC LIMIT 1"
    )
    if r:
...
def gen_html(d, contacts, path):
    ...
    if top:
        ...
        # Slide 6: Top 5
        top5_html = ''.join([
            f'<div class="rank-item"><span class="rank-num">{i}</span><span class="rank-name">{n(h)}</span><span class="rank-count">{t:,}</span></div>'
            for i,(h,t,_,_) in enumerate(top[:5],1)
        ])
        slides.append(f'''
        <div class="slide">
    ...
    if d['heating']:
        heat_html = ''.join([
            f'<div class="rank-item"><span class="rank-num">🔥</span><span class="rank-name">{n(h)}</span><span class="rank-count green">+{h2-h1}</span></div>'
            for h,h1,h2 in d['heating'][:5]
        ])
        slides.append(f'''
        <div class="slide orange-bg">
    ...
    if d['ghosted']:
        ghost_html = ''.join([
            f'<div class="rank-item"><span class="rank-num">👻</span><span class="rank-name">{n(h)}</span><span class="rank-count"><span class="green">{b}</span> → <span class="red">{a}</span></span></div>'
            for h,b,a in d['ghosted'][:5]
        ])
        slides.append(f'''
        <div class="slide">
...
#new part
                # GIFs & stickers
    mu = d.get('media_usage')
    if mu and (mu['gif_sent'] or mu['gif_recv'] or mu['sticker_sent'] or mu['sticker_recv']):
        gif_total = mu['gif_sent'] + mu['gif_recv']
        sticker_total = mu['sticker_sent'] + mu['sticker_recv']
        slides.append(f'''
        <div class="slide">
            <div class="slide-label">// CHAOTIC MEDIA</div>
            <div class="slide-text">how unhinged your reactions were</div>
            <div class="stat-grid">
                <div class="stat-item">
                    <span class="stat-num">{gif_total:,}</span>
                    <span class="stat-lbl">GIFs</span>
                </div>
                <div class="stat-item">
                    <span class="stat-num">{sticker_total:,}</span>
                    <span class="stat-lbl">stickers</span>
                </div>
            </div>
            <div class="roast" style="margin-top:24px;">
                you sent {mu['gif_sent']:,} GIFs and {mu['sticker_sent']:,} stickers yourself
            </div>
            <button class="slide-save-btn" onclick="saveSlide(this.parentElement, 'wrapped_media_chaos.png', this)">📸 Save</button>
...
def main():
    ...
    parser.add_argument('--use-2024', action='store_true',
                        help='Analyze 2024 instead of 2025')
    parser.add_argument('--db', help='Path to ChatStorage.sqlite (overrides auto-detect)')
    args = parser.parse_args()

    print("\n" + "="*50)
    print("  WhatsApp WRAPPED 2025")
    print("="*50 + "\n")

    print("[*] Checking access...")
    check_access(db_override=args.db)
    ...
    try:
        ...
        print("    (Couldn't auto-open, but the file was created. Open it manually.)")
